...
Effects.prototype = {
	...
	// Properties
	position:Vector.create( 500, 500 ),
	...
	setDefault: function( index ){
		...
		this.gravity = Vector.create( parseFloat(dados[20]), parseFloat(dados[21]) );
		
		this.positionRandom = Vector.create( parseFloat(dados[22]), parseFloat(dados[23]) );
...
//				currentParticle.position = Vector.add( currentParticle.position, Vector.create(-5,-5) );
	...
	draw: function( ){
		...
		for( var i = 0, j = this.particleCount; i < j; i++ ){
			...
			var radgrad = this.context.createRadialGradient( x + halfSize, y + halfSize, particle.sizeSmall, x + halfSize, y + halfSize, halfSize);
